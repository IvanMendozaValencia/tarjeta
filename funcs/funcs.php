<?php
	...
	function registraUsuario($usuario, $pass_hash, $nombre, $email, $clues, $activo, $token, $tipo_usuario){
		
		global $mysqli;
		
		$stmt = $mysqli->prepare("INSERT INTO usuarios (usuario, password, nombre, correo, clues_id, activacion, token, id_tipo) VALUES(?,?,?,?,?,?,?,?)");
		$stmt->bind_param('sssssisi', $usuario, $pass_hash, $nombre, $email, $clues, $activo, $token, $tipo_usuario);
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = '369sistimv@gmail.com'; //369sistimv@gmail.com
		$mail->Password = 'jktu tvzg ubmq ooeq'; //contraseñas de la aplicación
		
		$mail->setFrom('369sistimv@gmail.com', 'SNSP-Guerrero - Sistema de la Tarjeta de Atención Integral del Embarazo, Puerperio y Periodo de Lactancia | Servicio Nacional De Salud Pública - Coordinación Guerrero');
		...
		if($rows > 0) {
			...
			if($activacion == 1){
				...
				if(activarUsuario($id)){
					$msg = 'Tu cuenta se activó con éxito.';
					} else {
